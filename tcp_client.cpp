...
bool TCPClient::receive_packet(PacketHeader& header, std::vector<uint8_t>& payload) {
    ...
    if (!deserialize_header(header_data, header)) {
        std::cerr << "client: Failed to deserialize header" << std::endl;
        return false;
    }

    // 检查协议版本
    if (header.version != PROTOCOL_VERSION) {
        std::cerr << "client: Unsupported protocol version" << std::endl;
        return false;
    }
    ...
    if (header.payload_size > 0) {
        ...
        while (static_cast<size_t>(total_received) < payload.size()) {
            ssize_t bytes_received = recv(sockfd, data_ptr + total_received, remaining, 0);
...
bool TCPClient::upload_file(const std::string& local_path, 
    ...
    if (!fs::exists(local_path) || !fs::is_regular_file(local_path)) {
        std::cerr << "client: File not found or is not a regular file: " << local_path << std::endl;
        return false;
    }
    ...
    if (!receive_packet(header, response_payload)) {
        std::cerr << "client: Failed to receive upload response" << std::endl;
        return false;
    }

    if (header.command != CommandType::UPLOAD_RESPONSE) {
        std::cerr << "client: Unexpected response to upload request" << std::endl;
        return false;
    }

    UploadResponse resp;
    if (!deserialize_upload_response(response_payload, resp)) {
        std::cerr << "client: Failed to deserialize upload response" << std::endl;
        return false;
    }

    if (!resp.accepted) {
        std::cerr << "client: Upload rejected. Error code: " << static_cast<int>(resp.error_code) << std::endl;
        return false;
    }
    ...
    if (!file) {
        std::cerr << "client: Failed to open file for reading: " << local_path << std::endl;
        return false;
    }
    ...
    for (uint32_t i = start_block; i < total_blocks; ++i) {
        ...
        if (!send_packet(header, block.data)) {
            std::cerr << "client: Failed to send data block " << i << std::endl;
            return false;
        }
        ...
        if (!receive_packet(header, ack_payload)) {
            std::cerr << "client: Failed to receive ACK for block " << i << std::endl;
            return false;
        }

        if (header.command != CommandType::BLOCK_ACK) {
            std::cerr << "client: Unexpected response to data block " << i << std::endl;
            return false;
        }
        ...
        if (!deserialize_block_ack(ack_payload, ack)) {
            std::cerr << "client: Failed to deserialize block ack" << std::endl;
            return false;
        }

        if (!ack.success) {
            std::cerr << "client: Block " << i << " rejected. Error code: " << static_cast<int>(ack.error_code) << std::endl;
            return false;
        }
    ...
    if (!send_packet(header, serialize_transfer_complete(complete))) {
        std::cerr << "client: Failed to send transfer complete notification" << std::endl;
        return false;
    }
...
bool TCPClient::download_file(const std::string& remote_filename,
    ...
    std::vector<uint8_t> response_payload;
    if (!receive_packet(header, response_payload)) {
        std::cerr << "client: Failed to receive download response" << std::endl;
        return false;
    }

    if (header.command != CommandType::DOWNLOAD_RESPONSE) {
        std::cerr << "client: Unexpected response to download request" << std::endl;
        return false;
    }

    DownloadResponse resp;
    if (!deserialize_download_response(response_payload, resp)) {
        std::cerr << "client: Failed to deserialize download response" << std::endl;
        return false;
    }

    // 使用 resp.found 字段进行判断
    if (!resp.found) {
        std::cerr << "client: Download rejected. Error code: " << static_cast<int>(resp.error_code) << std::endl;
        return false;
    }
    ...
    if (!file) {
        std::cerr << "client: Failed to open file for writing: " << local_path << std::endl;
        return false;
    }
    ...
    for (uint32_t i = 0; i < total_blocks; ++i) {
        ...
        if (!receive_packet(header, data_block_payload)) {
            std::cerr << "client: Failed to receive data block " << i << std::endl;
            return false;
        }

        if (header.command != CommandType::DATA_BLOCK || header.block_index != i) {
            std::cerr << "client: Unexpected response for block " << i << std::endl;
            return false;
        }
        ...
        if (!send_packet(ack_header, serialize_block_ack(ack))) {
            std::cerr << "client: Failed to send ACK for block " << i << std::endl;
            return false;
        }
    ...
    if (!receive_packet(header, complete_payload)) {
        std::cerr << "client: Failed to receive transfer complete notification" << std::endl;
        return false;
    }

    if (header.command != CommandType::TRANSFER_COMPLETE) {
        std::cerr << "client: Unexpected response after all blocks received" << std::endl;
        return false;
    }

    TransferComplete complete_data;
    if (!deserialize_transfer_complete(complete_payload, complete_data)) {
        std::cerr << "client: Failed to deserialize transfer complete" << std::endl;
        return false;
    }
    ...
    if (file_hash_local != file_hash_remote) {
        std::cerr << "client: File hash mismatch! Downloaded file may be corrupted." << std::endl;
        return false;
    }
